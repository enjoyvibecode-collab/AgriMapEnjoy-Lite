...
function updateFertilizerDashboard() {
  ...
  // Save for the mobile premium PM calculator
  window.lastCalculatedAverages = { ph: avgPh, n: avgN, p: avgP, k: avgK, c: avgC };
  ...
  // Live recalculate premium PM farm calculations
  if (window.recalculateFarmFinance) {
    window.recalculateFarmFinance();
  }
}

// Global scope Premium PM Farm Financial & Yield projections calculator
window.selectedCalcCrop = 'cabai';

window.toggleCalcCrop = function(crop) {
  window.selectedCalcCrop = crop;
  const tabCabai = document.getElementById('calc-tab-cabai');
  const tabMentimun = document.getElementById('calc-tab-mentimun');
  
  if (crop === 'cabai') {
    if (tabCabai) tabCabai.className = "px-3 py-1 rounded text-[10px] font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer text-slate-100 bg-emerald-600 shadow-sm";
    if (tabMentimun) tabMentimun.className = "px-3 py-1 rounded text-[10px] font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer text-slate-400 hover:text-slate-100";
    
    // Default prices for Cabai Rawit
    const hargaInput = document.getElementById('calc-input-harga');
    const hargaRange = document.getElementById('calc-range-harga');
    if (hargaInput) hargaInput.value = 35000;
    if (hargaRange) {
      hargaRange.min = 5000;
      hargaRange.max = 80000;
      hargaRange.value = 35000;
    }
  } else {
    if (tabCabai) tabCabai.className = "px-3 py-1 rounded text-[10px] font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer text-slate-400 hover:text-slate-100";
    if (tabMentimun) tabMentimun.className = "px-3 py-1 rounded text-[10px] font-extrabold uppercase tracking-wider transition-all duration-200 cursor-pointer text-slate-100 bg-emerald-600 shadow-sm";
    
    // Default prices for Cucumber
    const hargaInput = document.getElementById('calc-input-harga');
    const hargaRange = document.getElementById('calc-range-harga');
    if (hargaInput) hargaInput.value = 14000;
    if (hargaRange) {
      hargaRange.min = 2000;
      hargaRange.max = 30000;
      hargaRange.value = 14000;
    }
  }
  window.recalculateFarmFinance();
};

window.syncCalcInputs = function(type, val) {
  if (type === 'luas') {
    const rangeLuas = document.getElementById('calc-range-luas');
    if (rangeLuas) rangeLuas.value = val;
  } else if (type === 'luas-val') {
    const inputLuas = document.getElementById('calc-input-luas');
    if (inputLuas) inputLuas.value = val;
  } else if (type === 'harga') {
    const rangeHarga = document.getElementById('calc-range-harga');
    if (rangeHarga) rangeHarga.value = val;
  } else if (type === 'harga-val') {
    const inputHarga = document.getElementById('calc-input-harga');
    if (inputHarga) inputHarga.value = val;
  }
  window.recalculateFarmFinance();
};

window.recalculateFarmFinance = function() {
  const crop = window.selectedCalcCrop || 'cabai';
  const luas = parseFloat(document.getElementById('calc-input-luas')?.value) || 1.0;
  const harga = parseFloat(document.getElementById('calc-input-harga')?.value) || (crop === 'cabai' ? 35000 : 14000);
  const biayaTetapTotal = (parseFloat(document.getElementById('calc-input-biaya-tetap')?.value) || 18000000) * luas;

  // Retrieve standard average parameters
  const avgs = window.lastCalculatedAverages || { ph: 6.5, n: 0.25, p: 25.0, k: 125.0, c: 1.8 };
  
  // Calculate raw fertilizer dosage per Ha using getFertilizerRecommendation
  const rec = getFertilizerRecommendation(avgs.ph, avgs.n, avgs.p, avgs.k, avgs.c);

  const doseUreaKg = (rec?.dosages?.urea?.val !== undefined ? rec.dosages.urea.val : 140) * luas; 
  const doseSP36Kg = (rec?.dosages?.sp36?.val !== undefined ? rec.dosages.sp36.val : 120) * luas; 
  const doseKClKg = (rec?.dosages?.kcl?.val !== undefined ? rec.dosages.kcl.val : 100) * luas; 
  const doseDolomitKg = (rec?.dosages?.dolomit?.val !== undefined ? rec.dosages.dolomit.val : 1.5) * 1000 * luas; 
  const doseOrganikKg = ((rec?.dosages?.organik?.val !== undefined ? rec.dosages.organik.val : rec?.dosages?.kompos?.val) || 10) * 1000 * luas; 

  // Standard Indonesian prices per kg/Ton
  const priceUrea = 4500;
  const priceSP36 = 5500;
  const priceKCl = 9000;
  const priceDolomit = 1200; // per kg (Rp 1.200.000 / Ton)
  const priceOrganik = 800;   // per kg (Rp 800.000 / Ton)

  const costUrea = doseUreaKg * priceUrea;
  const costSP36 = doseSP36Kg * priceSP36;
  const costKCl = doseKClKg * priceKCl;
  const costDolomit = doseDolomitKg * priceDolomit;
  const costOrganik = doseOrganikKg * priceOrganik;

  const totalCostPupuk = costUrea + costSP36 + costKCl + costDolomit + costOrganik;
  const grandTotalCost = totalCostPupuk + biayaTetapTotal;

  // Calculate Soil pH Efficiency multiplier (Yield efficiency)
  let eff = 1.0;
  let effDesc = "Kondisi hara tanah melimpah ruah & optimal.";
  let effEmoji = "✨";
  
  if (avgs.ph < 5.0 || avgs.ph > 8.0) {
    eff = 0.55;
    effDesc = "pH ekstrem mengunci keaktifan hara mikro penting (Hasil Anjlok ~45%). Perlu kapur!";
    effEmoji = "⚠️";
  } else if (avgs.ph < 5.5) {
    eff = 0.70;
    effDesc = "Lahan sangat masam mengunci hara makro utama (Penurunan hasil ~30%). Segera tabur Dolomit!";
    effEmoji = "🚜";
  } else if (avgs.ph >= 5.5 && avgs.ph < 6.0) {
    eff = 0.88;
    effDesc = "Tanah agak masam, penyerapan pupuk ringan terhambat (Penurunan hasil ~12%).";
    effEmoji = "🌿";
  } else if (avgs.ph > 7.5) {
    eff = 0.80;
    effDesc = "Tanah cenderung basa mengunci zat besi alami (Penurunan hasil ~20%).";
    effEmoji = "🪵";
  }

  // Potential yield / Ha
  // Chili Rawit: 13 Ton/Ha
  // Mentimun: 22 Ton/Ha
  const baseYieldPerHa = crop === 'cabai' ? 13.0 : 22.0;
  const totalYieldTons = baseYieldPerHa * eff * luas;

  // Total gross income
  const totalYieldKg = totalYieldTons * 1000;
  const grossIncome = totalYieldKg * harga;
  const netProfit = grossIncome - grandTotalCost;
  const roi = grandTotalCost > 0 ? (netProfit / grandTotalCost) * 100 : 0;

  // Update DOM Elements
  const effPercentEl = document.getElementById('calc-efficiency-percent');
  const effDescEl = document.getElementById('calc-efficiency-desc');
  const effEmojiEl = document.getElementById('calc-efficiency-emoji');

  if (effPercentEl) {
    effPercentEl.textContent = `${(eff * 100).toFixed(0)}%`;
    if (eff >= 0.90) {
      effPercentEl.className = "w-7 h-7 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full flex items-center justify-center text-[10px] font-black";
    } else if (eff >= 0.75) {
      effPercentEl.className = "w-7 h-7 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-full flex items-center justify-center text-[10px] font-black";
    } else {
      effPercentEl.className = "w-7 h-7 bg-red-500/10 border border-red-500/20 text-red-400 rounded-full flex items-center justify-center text-[10px] font-black";
    }
  }
  if (effDescEl) effDescEl.textContent = effDesc;
  if (effEmojiEl) effEmojiEl.textContent = effEmoji;

  // Render Metric A: Tonase Panen
  const metricOutput = document.getElementById('calc-metric-output');
  const metricOutputDetail = document.getElementById('calc-metric-output-detail');
  if (metricOutput) {
    metricOutput.innerHTML = `${totalYieldTons.toFixed(2)} <span class="text-[10px] font-bold text-slate-450">Ton</span>`;
  }
  if (metricOutputDetail) {
    metricOutputDetail.textContent = `Target: ${baseYieldPerHa} Ton/Ha (Aktual: ${(baseYieldPerHa * eff).toFixed(1)} T/Ha)`;
  }

  // Render Metric B: Kebutuhan Modal
  const metricCapital = document.getElementById('calc-metric-capital');
  const metricCapitalDetail = document.getElementById('calc-metric-capital-detail');
  if (metricCapital) {
    if (grandTotalCost >= 1000000) {
      metricCapital.innerHTML = `<span class="text-xxs font-extrabold text-slate-400">Rp</span> ${(grandTotalCost / 1000000).toFixed(2)}<span class="text-xs">Jt</span>`;
    } else if (grandTotalCost >= 1000) {
      metricCapital.innerHTML = `<span class="text-xxs font-extrabold text-slate-400">Rp</span> ${(grandTotalCost/1000).toFixed(0)}K`;
    } else {
      metricCapital.innerHTML = `<span class="text-xxs font-extrabold text-slate-400">Rp</span> ${grandTotalCost}`;
    }
  }
  if (metricCapitalDetail) {
    metricCapitalDetail.textContent = `Ppk: Rp ${(totalCostPupuk/1000000).toFixed(2)}Jt | Bud: Rp ${(biayaTetapTotal/1000000).toFixed(2)}Jt`;
  }

  // Render Metric C: Laba Bersih
  const metricProfit = document.getElementById('calc-metric-profit');
  const metricProfitDetail = document.getElementById('calc-metric-profit-detail');
  const cardProfitBtn = document.getElementById('calc-card-profit');
  const profitTitle = document.getElementById('calc-metric-profit-title');

  if (profitTitle) {
    profitTitle.textContent = netProfit >= 0 ? "📈 Laba Bersih" : "📉 Kerugian Lahan";
  }

  if (metricProfit) {
    const sign = netProfit >= 0 ? '+' : '';
    if (Math.abs(netProfit) >= 1000000) {
      metricProfit.innerHTML = `<span class="text-xxs font-extrabold text-slate-400">Rp</span> ${sign}${(netProfit / 1000000).toFixed(2)}<span class="text-xs">Jt</span>`;
    } else if (Math.abs(netProfit) >= 1000) {
      metricProfit.innerHTML = `<span class="text-xxs font-extrabold text-slate-400">Rp</span> ${sign}${(netProfit/1000).toFixed(0)}K`;
    } else {
      metricProfit.innerHTML = `<span class="text-xxs font-extrabold text-slate-400">Rp</span> ${sign}${netProfit.toFixed(0)}`;
    }
    
    if (netProfit >= 0) {
      if (cardProfitBtn) cardProfitBtn.className = "bg-emerald-950/65 p-3.5 rounded-2xl border border-emerald-800/80 flex flex-col justify-between h-[90px]";
      metricProfit.className = "text-lg font-black text-emerald-300 m-0 p-0 border-none leading-none flex items-baseline gap-0.5";
    } else {
      if (cardProfitBtn) cardProfitBtn.className = "bg-red-950/65 p-3.5 rounded-2xl border border-red-800/80 flex flex-col justify-between h-[90px]";
      metricProfit.className = "text-lg font-black text-red-300 m-0 p-0 border-none leading-none flex items-baseline gap-0.5";
    }
  }

  if (metricProfitDetail) {
    const roiSign = roi >= 0 ? '+' : '';
    let recommendationAdviceText = "";
    if (roi >= 120) recommendationAdviceText = "ROI Menjanjikan!";
    else if (roi >= 50) recommendationAdviceText = "Margin Sehat.";
    else if (roi >= 0) recommendationAdviceText = "Untung Tipis.";
    else recommendationAdviceText = "Rugi! Rawat pH.";
    
    metricProfitDetail.textContent = `ROI: ${roiSign}${roi.toFixed(0)}% (${recommendationAdviceText})`;
  }

  // Update details calculation table
  const itemUrea = document.getElementById('calc-item-urea');
  const itemSP36 = document.getElementById('calc-item-sp36');
  const itemKCl = document.getElementById('calc-item-kcl');
  const itemDolomit = document.getElementById('calc-item-dolomit');
  const itemOrganik = document.getElementById('calc-item-organik');
  const totalPupukText = document.getElementById('calc-item-total-pupuk');
  ...
  if (itemUrea) {
    itemUrea.innerHTML = `<span>• Pupuk Nitrogen (Urea)</span> <span class="font-mono text-xs font-bold text-slate-100">${doseUreaKg.toFixed(0)} kg x Rp${priceUrea.toLocaleString('id-ID')} = ${formatRupiah(costUrea)}</span>`;
  }
  if (itemSP36) {
    itemSP36.innerHTML = `<span>• Pupuk Fosfat (SP-36)</span> <span class="font-mono text-xs font-bold text-slate-100">${doseSP36Kg.toFixed(0)} kg x Rp${priceSP36.toLocaleString('id-ID')} = ${formatRupiah(costSP36)}</span>`;
  }
  if (itemKCl) {
    itemKCl.innerHTML = `<span>• Pupuk Kalium (KCl)</span> <span class="font-mono text-xs font-bold text-slate-100">${doseKClKg.toFixed(0)} kg x Rp${priceKCl.toLocaleString('id-ID')} = ${formatRupiah(costKCl)}</span>`;
  }
  if (itemDolomit) {
    itemDolomit.innerHTML = `<span>• Kapur Pertanian (Dolomit)</span> <span class="font-mono text-xs font-bold text-slate-100">${(doseDolomitKg/1000).toFixed(1)} t x Rp${(priceDolomit*1000).toLocaleString('id-ID')} = ${formatRupiah(costDolomit)}</span>`;
  }
  if (itemOrganik) {
    itemOrganik.innerHTML = `<span>• Kotoran Hewani / Organik</span> <span class="font-mono text-xs font-bold text-slate-100">${(doseOrganikKg/1000).toFixed(1)} t x Rp${(priceOrganik*1000).toLocaleString('id-ID')} = ${formatRupiah(costOrganik)}</span>`;
  }
  if (totalPupukText) {
    totalPupukText.textContent = formatRupiah(totalCostPupuk);
  }
};
